...
def main():
    ...
    with open('/mnt/big_narstie_data/chenxi/data/medium_dataset_normal_wall/environments_' + str(environment_type), 'r') as env_file:
        ...
        for environment_index in range(NUM_ENVIRONMENT_PER_TYPE):
            if os.path.exists('/mnt/big_narstie_data/chenxi/data/medium_dataset_normal_wall/dynamic_cost_' + str(environment_type) + '_' + str(environment_index)):
                ...
                with open('/mnt/big_narstie_data/chenxi/data/medium_dataset_normal_wall/dynamic_cost_' + str(environment_type) + '_' + str(environment_index), 'r') as file:
                    ...
                    for p1 in p1_list:
                        ...
                        for p2 in p2_list:
                            ...
                            if environment_index < 40:
                                training.append(example_id)
                            elif environment_index < 45:
            ...
            elif os.path.exists('/mnt/big_narstie_data/chenxi/data/medium_dataset_normal_wall/dynamic_cost_plus_type_' + str(environment_type) + '_' + str(environment_index)):
                ...
                with open('/mnt/big_narstie_data/chenxi/data/medium_dataset_normal_wall/dynamic_cost_plus_type_' + str(environment_type) + '_' + str(environment_index), 'r') as file:
                    ...
                    for p1 in p1_list:
                        ...
                        for p2 in p2_list:
                            example_id = depth_map_id + '_' + str(p2[0]) + str(p2[1]) + str(p2[2])  
                            if environment_index < 16:
                                training.append(example_id)
                            elif environment_index < 18:
                                validation.append(example_id)
                            else:
                                test.append(example_id)
                            ...
                            p2_ddyn_dict[example_id] = (depth_map_id, np.array(p2).astype(np.float32), np.percentile(np.array(ddyn_list), PERCENTILE).astype(np.float32))
    
    with open('/mnt/big_narstie_data/chenxi/data/ground_truth_p1p2/p2_ddyn_' + str(environment_type), 'w') as file:
        pickle.dump(p2_ddyn_dict, file)

    partition = {'training': training,
                 'validation': validation,
                 'test': test}
    with open('/mnt/big_narstie_data/chenxi/data/ground_truth_p1p2/partition_' + str(environment_type), 'w') as file:
