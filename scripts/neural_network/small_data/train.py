...
def main():
    with open('/mnt/big_narstie_data/chenxi/data/ground_truth_p1p2/p2_ddyn_0', 'r') as file:
        p2_ddyn = pickle.load(file)

    with open('/mnt/big_narstie_data/chenxi/data/ground_truth_p1p2/partition_0', 'r') as file:
        partition = pickle.load(file)
    print('number of training examples: {}'.format(len(partition['training'][:1])))
    print(partition['training'][:1])
    # print('number of validation examples: {}'.format(len(partition['validation'])))

    torch.manual_seed(20190711)
    training_dataset = Dataset(p2_ddyn, partition['training'][:1])
