...
class Model(nn.Module):
    def __init__(self):
        ...
        self.ground_net = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=7, padding=0),
            nn.LeakyReLU(),
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=7, padding=0),
            nn.LeakyReLU(),
            nn.Conv2d(in_channels=32, out_channels=32, kernel_size=7, padding=0),
            nn.LeakyReLU(),
    ...
    def forward(self, ground_depth_map, wall_depth_map, p2):
        ...
        arr1 = arr1.view(-1, 32, 11*11)
        p2_ground = self.p2_net_ground(p2).view(-1, 11*11, 5)
        arr1 = torch.bmm(arr1, p2_ground).view(-1, 32*5)
        arr2 = self.wall_net(wall_depth_map)
        arr2 = arr2.view(-1, 32, 3*117)
        p2_wall = self.p2_net_wall(p2).view(-1, 3*117, 5)
