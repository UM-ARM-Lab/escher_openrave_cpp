...
class Model(nn.Module):
    ...
    def forward(self, ground_depth_map, wall_depth_map, p2):
        ...
        arr1 = arr1.view(-1, 64, 9*9)
        p2_ground = self.p2_net_ground(p2).view(-1, 9*9, 5)
        arr1 = torch.bmm(arr1, p2_ground).view(-1, 64*5)
        arr2 = self.wall_net(wall_depth_map)
        arr2 = arr2.view(-1, 64, 5*119)
        p2_wall = self.p2_net_wall(p2).view(-1, 5*119, 5)
