...
def sample_contact_transitions(env_handler,robot_obj,hand_transition_model1, hand_transition_model2, foot_transition_model1, foot_transition_model2,structures,grid_resolution,environment_index,transitions,args):
    init_node_list = []

    rave.raveLogInfo('Orientation: ' + repr(args.p1yaw) + ' degrees.')
    orientation_rad = args.p1yaw * DEG2RAD
    orientation_rotation_matrix = rpy_to_SO3([0, 0, args.p1yaw])
    ...
    dummy_init_left_leg = [-0.1*math.sin(orientation_rad),0.1*math.cos(orientation_rad),0,0,0,args.p1yaw]
    dummy_init_right_leg = [0.1*math.sin(orientation_rad),-0.1*math.cos(orientation_rad),0,0,0,args.p1yaw]
    ...
    for init_left_hand_pose in init_left_hand_pose_lists:
        for init_right_hand_pose in init_right_hand_pose_lists:
            ...
            for foot_transition in foot_transition_model1:
                ...
                init_node.left_leg = [init_left_foot_position[0,0], init_left_foot_position[1,0], LARGE_NUMBER, 0, 0, foot_transition[2]/2 + args.p1yaw]
                init_node.right_leg = [init_right_foot_position[0,0], init_right_foot_position[1,0], LARGE_NUMBER, 0, 0, -foot_transition[2]/2 + args.p1yaw]
    ...
    for init_node in init_node_list:
        ...
        for child_node in child_node_list:
            ...
            if temp_dict['p2'][0] > GRID_RESOLUTION*(args.dx-0.5) and temp_dict['p2'][0] < GRID_RESOLUTION*(args.dx+0.5) and \
                temp_dict['p2'][1] > GRID_RESOLUTION*(args.dy-0.5) and temp_dict['p2'][1] < GRID_RESOLUTION*(args.dy+0.5):
                if (temp_dict['p2'][5] > args.p1yaw+ANGLE_RESOLUTION*(args.dyaw-0.5) and temp_dict['p2'][5] < args.p1yaw+ANGLE_RESOLUTION*(args.dyaw+0.5)) or \
                    (temp_dict['p2'][5] > args.p1yaw+ANGLE_RESOLUTION*(args.dyaw-0.5)-360 and temp_dict['p2'][5] < args.p1yaw+ANGLE_RESOLUTION*(args.dyaw+0.5)-360) or \
                        (temp_dict['p2'][5] > args.p1yaw+ANGLE_RESOLUTION*(args.dyaw-0.5)+360 and temp_dict['p2'][5] < args.p1yaw+ANGLE_RESOLUTION*(args.dyaw+0.5)+360):
...
def sample_env(env_handler, robot_obj, p1x, p1y, surface_source):
    env_handler.update_environment(robot_obj, p1x, p1y, surface_source=surface_source)
...
def main(robot_name='athena'): # for test
    ...
    parser.add_argument('--p1x', type=float)
    parser.add_argument('--p1y', type=float)
    parser.add_argument('--p1yaw', type=float)
    parser.add_argument('--dx', type=int)
    parser.add_argument('--dy', type=int)
    parser.add_argument('--dyaw', type=int)
    args = parser.parse_args()

    # p1x = -0.45 # p1x should be position, not index, remember to modify environment_handler
    # p1y = 2.55 # p1y should be position, not index, remember to modify environment handler
    # p1yaw = 0.0
    # dx = 0 # should be index, not pisition
    # dy = 1 # should be index, not position
    # dyaw = 0 # should be index, not position

    # sample environments and contact transitions
    structures = sample_env(env_handler, robot_obj, args.p1x, args.p1y, 'torso_path_planning_test_env_13')

    # save all transitions to this list
    transitions = []
    sample_contact_transitions(env_handler, robot_obj, hand_transition_model1, hand_transition_model2, foot_transition_model1, foot_transition_model2, structures, GRID_RESOLUTION, 9999, transitions, args)
    # IPython.embed()
    with open('specific_transitions', 'w') as file:
