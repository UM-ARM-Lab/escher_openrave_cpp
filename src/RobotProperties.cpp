...
RobotProperties::RobotProperties(OpenRAVE::RobotBasePtr _robot, std::vector<dReal> _IK_init_DOF_Values, std::vector<dReal> _default_DOF_Values, 
    ...
    hand_h_(_hand_h),
    hand_w_(_hand_w),
    hand_radious_(std::sqrt(_hand_h*_hand_h + _hand_w*_hand_w)),
    robot_z_(_robot_z),
    top_z_(_top_z),
    ...
    shoulder_w_(_shoulder_w)
{
    ...
    {
        DOFName_index_map_.insert(std::make_pair<string,int>(joint->GetName(), joint->GetJointIndex()));
    }

    active_dof_index = 0
    for(auto &dof_index : _robot->GetActiveDOFIndices())
    {
        ActiveDOFName_index_map_.insert(std::make_pair<string,int>(_robot->GetJointFromDOFIndex(dof_index)->GetName(), active_dof_index));
        active_dof_index++;
    }
