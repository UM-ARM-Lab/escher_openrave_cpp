...
class EscherMotionPlanning : public OpenRAVE::ModuleBase
{
    ...
    private:
        ...
        std::map<std::array<int,5>,float> calculateFootstepTransitionTraversability(std::vector<std::array<int,5>> torso_transitions);
        ...
        std::map< std::array<int,3>, std::array<float,4> > calculateHandTransitionTraversability(std::vector< GridIndices3D > torso_poses);
